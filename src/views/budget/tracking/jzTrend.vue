  ...
  <div class="jzTrend">
    <div class="operation">
      <el-form :inline="true" :model="form" label-width="80px" class="formList">
        ...
        <el-form-item>
          <el-button size="small" type="primary" @click="findBusinessIndexGrowth">查询</el-button>
        </el-form-item>
    ...
    <div class="tableBox">
      <el-table v-loading="tableLoading" :data="tableData" style="width: 100%" class="tableTh28" border stripe height="65vh" :span-method="arraySpanMethod" ref="table">
        ...
        <el-table-column label="净增">
          ...
          <el-table-column prop="indexCompleteSum" label="累计完成数" width="120"></el-table-column>
        </el-table-column>
        <el-table-column label="净增季度累计完成数">
        ...
        <el-table-column label="净增季度指标累计完成率">
          ...
          <el-table-column prop="rateQuarterFour" label="四季度" class-name="right-money"></el-table-column>
        </el-table-column>
      </el-table>
    </div>
  </div>
    ...
    methods:{
      ...
      exportExcel(){
        ...
        if (process.env.NODE_ENV == 'development') {
          window.location.href = prefixAPI.loc + "/api/budgetTrack/ExportBusinessIndexByGrowth?areaId=" + areaId +'&month=' + year;
        } else if (process.env.NODE_ENV == 'production') {
          window.location.href = prefixAPI.hp + "/api/budgetTrack/ExportBusinessIndexByGrowth?areaId=" + areaId +'&month=' + year;
        }
      },
      //获取当前时间
      ...
      //净增趋势
      findBusinessIndexGrowth(params){
        this.tableData = [];
        let obj = {};
        if(this.form.areaId){
          obj = this.form
        ...
        $budgetAPI.findBusinessIndexGrowth(obj,res=>{
          this.alertError(res);
          if(res.code == 0){
            this.count = res.content[0].count;
            res.content.map((item,index)=>{
              item.list.map((child,idx)=>{
                // if(child.product == 'VP008001'){
                //   this.$set(child,'productName','移动')
                // }
                // if(child.product == 'VP008002'){
                //   this.$set(child,'productName','宽带')
                // }
                // if(child.product == 'VP008003'){
      ...
      init(){
        ...
        this.findBusinessIndexGrowth(params)
      }
    }
  }
</script>
