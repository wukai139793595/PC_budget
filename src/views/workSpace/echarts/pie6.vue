<template>
  <div style="height:100%;width:100%;">
    <div class="canvas5" style="height:100%;width:100%;"></div>
  </div>
</template>
  ...
  methods: {
    ...
    getPie(val) {
      ...
      val.data.map((item,index)=>{
        ...
        obj.name = item.CUSTTYPENEW;
        obj.value = item.INCOMELJ;
        data.push(obj)
      })
      var myChart = this.$echarts.init(document.querySelector(".canvas5"));
      let option = {
        tooltip: {
          trigger: "item",
          formatter: "{b}: {c}万元 ({d}%)"
        ...
        series: [
          {
            ...
            // selectedMode: "single",
            radius: ['40%', '55%'],
            // color:["#3dd2df","#49A9EE","#1D8BDA"],
            color: ['#2fa777','#ebbd40','#49a9ee'],
            label: {
              normal: {
                formatter: "{b|{b}}  \n{d|{d}% } ",
                rich: {
