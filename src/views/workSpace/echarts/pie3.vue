<template>
  <div style="height:100%;width:100%;">
    <div class="canvas1" style="height:100%;width:100%;"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getPie(val) {
      const that = this;
      var myChart = this.$echarts.init(document.querySelector(".canvas1"));
      let option = {
        tooltip: {
          trigger: "item",
          formatter: "{b}: {c}万元 ({d}%)"
        ...
        series: [
          {
            ...
            // },
            data: [
              { value: this.findData(val.data,"YS21061"), name: "实体渠道新增数", itemStyle: { color: "#49a9ee" } },
              { value: this.findData(val.data,"YS21062"), name: "政企渠道新增数", itemStyle: { color: "#6dc467" } },
              { value: this.findData(val.data,"YS21063"), name: "电子渠道新增数", itemStyle: { color: "#ebbd40" } },
            ]
          },
          // {
          //   name: "分产品构成",
